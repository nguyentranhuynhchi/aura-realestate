"""
housing_data_pipeline.py
=========================================================================
Pipeline tiền xử lý dữ liệu Bất Động Sản — Phân khúc "NHÀ RIÊNG"

File này GOM CHÍNH XÁC toàn bộ logic đã được kiểm chứng qua 4 notebook thí
nghiệm gốc, không tự phát minh hay đơn giản hóa bất kỳ bước xử lý nào:

    - 01_text_parsing_and_feature_enhancement.ipynb  -> _stage_01_text_parsing
    - 02_three_source_data_splitting.ipynb           -> _stage_02_filter_house_segment
    - 04_advanced_housing_preprocessing.ipynb        -> _stage_04_advanced_preprocessing
    - 05_split_and_transform.ipynb                   -> _stage_05_transform_and_scale

THIẾT KẾ CHO MLOPS:
    - fit_transform(raw_df) : dùng CHO TẬP TRAIN. Sẽ tự học (median các cột
      bedrooms/bathrooms, StandardScaler) và LƯU LẠI artifacts xuống đĩa.
    - transform(raw_df)     : dùng CHO VALIDATION / TEST / DỮ LIỆU MỚI (serving).
      Sẽ NẠP LẠI đúng thống kê đã học ở bước train (không học lại), đảm bảo
      không có rò rỉ dữ liệu (data leakage) và tính nhất quán train/serving.

LƯU Ý QUAN TRỌNG (khác biệt so với bản nháp DataCleaner cũ — đã được sửa lại
cho khớp 100% với thực nghiệm):
    1. price_raw / area_raw phải được bóc tách bằng đúng hàm clean_price /
       clean_area của file 01 (phân biệt đơn vị "tỷ" và "triệu"), KHÔNG dùng
       regex trích số đơn giản (regex đơn giản sẽ hiểu sai đơn vị "triệu").
    2. bedrooms / bathrooms / floors phải qua clean_to_integer (file 01)
       trước, sau đó mới fillna theo đúng thứ tự của file 04.
    3. Giá trị median dùng để fillna bedrooms/bathrooms phải được TÍNH VÀ
       LƯU LẠI một lần duy nhất lúc train — không được tính lại trên từng
       batch dữ liệu mới lúc inference (nếu không sẽ sai lệch so với model
       đã học).
    4. legal_status / interior / house_direction phải qua đúng hàm gom nhóm
       (clean_legal / extract_interior_clean / clean_direction) của file 04
       trước khi map sang số, KHÔNG map trực tiếp từ text thô.
    5. Sau vòng lặp VIF tự động ở file 04, biến 'bedrooms' và 'total_rooms'
       đã bị loại bỏ vĩnh viễn khỏi tập đặc trưng cuối cùng (chỉ giữ lại
       area_raw, floors, bathrooms) — pipeline này cố định lại kết quả đó
       thay vì chạy lại VIF mỗi lần (VIF chỉ cần chạy một lần lúc nghiên cứu).
    6. StandardScaler ở file 05 được fit trên TOÀN BỘ các cột đặc trưng còn
       lại, bao gồm cả các cột one-hot dir_* — pipeline này giữ đúng hành vi
       đó (không loại trừ dir_* khỏi scaling như bản nháp cũ).
    7. One-Hot hướng nhà được ép về đủ 8 cột cố định (dir_Bắc, dir_Nam, ...,
       dir_Đông Nam) bất kể batch dữ liệu mới có đủ 8 hướng hay không, để
       tránh lệch cột đặc trưng khi gọi scaler.transform() lúc serving.
=========================================================================
"""
